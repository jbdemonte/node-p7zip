...
['-aoa', '-aos', '-aou', '-aot'].forEach((item) => {
  test(`No overwrite mode preset: ${item} `, async () => {
    tools.run = jest.fn().mockReturnValue(Promise.resolve([]));
    await fn('archive', null, null, item);
